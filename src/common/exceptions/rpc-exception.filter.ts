...
export class RpcExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger('RpcExceptionFilter');
    ...
    this.logger.error('Exception caught in Election-MS:', exception);
    ...
    return throwError(() => ({
      status: 500,
      message: 'Internal server error in Election service',
  ...
  private handlePrismaError(exception: any) {
    switch (exception.code) {
      case 'P2002':
        return {
          status: 409,
          message: 'Election or candidate already exists. Please check unique constraints.',
          error: 'Conflict'
        };
      case 'P2025':
        return {
